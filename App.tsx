...
const initializeSettings = (): AppSettings => {
  try {
    ...
    if (saved) {
      ...
      // Ensure numeric values are properly converted (fixes temperature/topP/topK not working)
      return {
        ...parsed,
        temperature: Number(parsed.temperature) || INITIAL_SETTINGS.temperature,
        topP: Number(parsed.topP) || INITIAL_SETTINGS.topP,
        topK: Number(parsed.topK) || INITIAL_SETTINGS.topK,
        topA: Number(parsed.topA) || INITIAL_SETTINGS.topA,
        repetitionPenalty: Number(parsed.repetitionPenalty) || INITIAL_SETTINGS.repetitionPenalty,
        maxOutputTokens: Number(parsed.maxOutputTokens) || INITIAL_SETTINGS.maxOutputTokens,
        minOutputLength: Number(parsed.minOutputLength) || INITIAL_SETTINGS.minOutputLength,
        backgroundBlur: Number(parsed.backgroundBlur) || INITIAL_SETTINGS.backgroundBlur,
        backgroundOpacity: Number(parsed.backgroundOpacity) || INITIAL_SETTINGS.backgroundOpacity,
        actionButtonOpacity: Number(parsed.actionButtonOpacity) || INITIAL_SETTINGS.actionButtonOpacity
      };
    }
  } catch (error) {
...
function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (user) {
        ...
        const { characters: cloudChars, error: charError } = await dataService.loadCharacters(user.id);
        if (!charError && cloudChars.length > 0) {
          setCharacters(cloudChars);
          console.log('Loaded characters from Supabase:', cloudChars.length);
        }

        const { settings: cloudSettings, error: settingsError } = await dataService.loadSettings(user.id);
  ...
  useEffect(() => {
    const saveToCloud = async () => {
      ...
      for (const char of characters) {
        await dataService.saveCharacter(currentUser.id, char);
      }
    };
  ...
  useEffect(() => {
    const saveSettingsToCloud = async () => {
      if (!currentUser) return;
      await dataService.saveSettings(currentUser.id, settings);
    };

    const timer = setTimeout(saveSettingsToCloud, 2000);
    return () => clearTimeout(timer);
      ...
      {isLoadingUserData && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm">
              <div className="flex flex-col items-center gap-4">
                  ...
                  <p className="text-zinc-400 text-sm">Loading your account data...</p>
              </div>
          </div>
      )}
