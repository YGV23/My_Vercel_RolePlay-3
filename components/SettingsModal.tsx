...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, settings, onSave, initialTab }) => {
  ...
  const toggleLorebook = (id: string) => {
      const updatedLorebooks = localSettings.globalLorebooks?.map(lb => {
          if (lb.id === id) {
              console.log(`[Global Lorebook Toggle] Toggling "${lb.name}" from ${lb.enabled} to ${!lb.enabled}`);
              return { ...lb, enabled: !lb.enabled };
          }
          return lb;
      }) || [];
      console.log('[Global Lorebook Toggle] Updated lorebooks:', updatedLorebooks.map(lb => ({ name: lb.name, enabled: lb.enabled })));
      handleChange('globalLorebooks', updatedLorebooks);
  };
