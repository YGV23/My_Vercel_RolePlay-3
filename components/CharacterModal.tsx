...
INSTRUCTION: Output ONLY the raw content for this field. Do not include explanations, markdown blocks (unless requested), or labels. Be creative and consistent.`;
  ...
  const toggleLorebook = (id: string) => {
      setFormData(prev => {
          const updated = {
              ...prev,
              lorebooks: prev.lorebooks?.map(lb => {
                  if (lb.id === id) {
                      console.log(`[Lorebook Toggle] Toggling "${lb.name}" from ${lb.enabled} to ${!lb.enabled}`);
                      return { ...lb, enabled: !lb.enabled };
                  }
                  return lb;
              })
          };
          console.log('[Lorebook Toggle] Updated lorebooks:', updated.lorebooks?.map(lb => ({ name: lb.name, enabled: lb.enabled })));
          return updated;
      });
  };
